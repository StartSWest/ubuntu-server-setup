...
print_summary() {
    ...
    echo ""
    echo -e "  ${YELLOW}⚠ SSH hardening NOT yet applied (password auth still enabled)${NC}"
    echo ""
    ...
    echo ""
    echo "3. Deploy your application and verify everything works"
    echo ""
    echo -e "${YELLOW}4. FINAL STEP: Harden SSH (run AFTER everything is working):${NC}"
    echo "   sudo bash harden-ssh.sh"
    echo ""
    log_warning "The harden-ssh.sh script will disable password authentication."
    log_warning "Only run it after your deployments are complete and tested!"
    echo ""
